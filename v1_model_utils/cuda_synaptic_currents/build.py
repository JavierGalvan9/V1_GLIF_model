...
def main():
    ...
    cc_object = HERE / "synaptic_current_ops.cc.o"
    cu_object = HERE / "synaptic_current_ops.cu.o"
    output = HERE / "_synaptic_current_ops.so"
    started = time.perf_counter()
    _run([cxx, "-std=c++17", "-O3", "-fPIC", "-c", HERE / "synaptic_current_ops.cc", "-o", cc_object, *compile_flags])
    _run([
        nvcc, "-std=c++17", "-O3", "-x", "cu", "-c",
        HERE / "synaptic_current_ops.cu.cc", "-o", cu_object,
        "-DGOOGLE_CUDA=1", "--expt-relaxed-constexpr", "--use_fast_math",
    ...
    _run([
        cxx, "-shared", "-O3", cc_object, cu_object, "-o", output,
