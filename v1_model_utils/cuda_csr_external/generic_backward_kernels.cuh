...
template <typename T, int kWarps, int kTile>
__global__ __launch_bounds__(32 * kWarps) void GenericWeightDirectKernel(
    int64_t batch_size, int64_t n_pre, const T* activity,
    ...
    uint32 splits, float* weight_grad) {
  ...
  const int target = ExtReverseBits<3>(sub & (kPerSlot - 1));
  for (uint32 base = start + warp * kTile; base < end; base += kGrain) {
    const uint32 edge_lane = base + lane;
    const uint32 my_pair = edge_lane < end ? ExtLoadIndex(pair_ids + edge_lane) : 0u;
...
#pragma unroll
          for (int index = 0; index < kPack; ++index) {
            const int64_t batch = batch_base + sub * kPack + index;
            if (batch < batch_size) {
              partial[step] += projected[static_cast<int64_t>(pair) * batch_size + batch] *
                               sample[index];
            }
          }
        }
    ...
    ExtButterflyReduce<kPerSlot / 2, 1>(partial, lane);
#pragma unroll
...
template <typename T, int kWarps, int kTile>
__global__ __launch_bounds__(32 * kWarps) void GenericActivityPackedKernel(
    int64_t batch_size, int64_t n_pre, const float* projected,
    const float* weights, const uint32* pair_ids, const uint32* edge_ids,
    const uint32* row_splits, const uint32* nonempty_rows, int64_t n_rows,
    uint32 splits, float* scratch, T* activity_grad) {
  constexpr int kPack = 4;
  constexpr int kLanesPerEdge = 8;
  constexpr int kSlots = 4;
  constexpr int kPerSlot = kTile / kSlots;
  ...
  if (start >= row_end) return;
  const uint32 end = min(row_end, start + chunks_per_split * kGrain);
  const int64_t batch_base = static_cast<int64_t>(blockIdx.z) * 32;
  float grad[kPack] = {};
  for (uint32 base = start + warp * kTile; base < end; base += kGrain) {
    const uint32 edge_lane = base + lane;
    const bool own = edge_lane < end;
    const uint32 my_pair = own ? ExtLoadIndex(pair_ids + edge_lane) : 0u;
    const float my_weight = own ? ExtLoadWeight(weights + V1_EDGE_INDEX(edge_lane)) : 0.0f;
#pragma unroll
    for (int step = 0; step < kPerSlot; ++step) {
      const int column = kSlots * step + slot;
      const uint32 pair = __shfl_sync(0xffffffff, my_pair, column);
...
#pragma unroll
        for (int index = 0; index < kPack; ++index) {
          const int64_t batch = batch_base + sub * kPack + index;
          if (batch < batch_size) {
            grad[index] += projected[static_cast<int64_t>(pair) * batch_size + batch] * weight;
          }
        }
      }
    }
...
#pragma unroll
    ...
    const int64_t offset = (batch_base + lane) * n_pre + pre;
