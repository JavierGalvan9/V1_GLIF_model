...
def compute_ks_statistics(df, metric='Weight', min_n_sample=15):
    ...
    for cell_type in cell_types:
        ...
        df1 = df.loc[(df['Post_names'] == cell_type) & (
            df['Weight Type'] == 'Initial weight'), metric]
        df2 = df.loc[(df['Post_names'] == cell_type) & (
            df['Weight Type'] == 'Final weight'), metric]
        # Drop NA values
...
def exponential_decay_filter(spike_train, tau=20):
    ...
    for i in range(1, len(spike_train)):
        continuous_signal[i] = decay_factor * \
            continuous_signal[i-1] + spike_train[i]
...
def pop_fano(spikes, bin_sizes):
    ...
    for i, bin_width in enumerate(bin_sizes):
        ...
        trimmed_spikes = np.reshape(
            trimmed_spikes, (max_index // bin_size, bin_size))
        # sum over the bins
...
class OsiDsiCallbacks:
    ...
    def save_inference_metrics(self):
        ...
        sem_step_time = np.std(self.inference_times) / \
            np.sqrt(len(self.inference_times))
        ...
        if gpu_memory_data:
            ...
            sem_gpu_memory = np.std(gpu_memory_data) / \
                np.sqrt(len(gpu_memory_data))
        else:
            mean_gpu_memory = 0.0
            sem_gpu_memory = 0.0
        ...
        sem_rate = np.std(self.inference_rates) / \
            np.sqrt(len(self.inference_rates))

        stats_file = tf_utils.append_performance_statistics(
            logdir=self.logdir,
            n_neurons=self.n_neurons,
            n_edges=self.n_edges,
            batch_size=self.flags.n_trials_per_angle,
            seq_len=self.sim_duration,
            mean_rate=mean_rate,
            sem_rate=sem_rate,
    ...
    def trainable_variable_change_heatmaps_and_distributions(self, variable):
        node_types_voltage_scale = (
            self.network['node_params']['V_th'] - self.network['node_params']['E_L']).astype(np.float32)
        ...
        if 'rest_of_brain_weights' in variable:
            ...
            self.node_to_pop_weights_analysis(
                self.bkg_input['indices'], variable=variable, voltage_scale=voltage_scale)
        elif 'sparse_input_weights' in variable:
            voltage_scale = node_types_voltage_scale[node_type_ids[self.lgn_input['indices'][:, 0]]]
            self.node_to_pop_weights_analysis(
                self.lgn_input['indices'], variable=variable, voltage_scale=voltage_scale)
            ...
            self.pop_to_pop_weights_analysis(
                indices, variable=variable, voltage_scale=voltage_scale)
            self.pop_to_pop_weights_distribution(
                indices, variable=variable, voltage_scale=voltage_scale)
    ...
    def node_to_pop_weights_analysis(self, indices, variable='', voltage_scale=None):
        ...
        target_cell_types = [other_v1_utils.pop_name_to_cell_type(
        ...
        df = pd.DataFrame({
            # Duplicate node names for initial and final weights
            'Post_names': post_cell_types * 2,
            # Combine initial and final weights
            'Weight': initial_weights.tolist() + final_weights.tolist(),
            # Distinguish between initial and final weights
            'Weight Type': ['Initial weight'] * len(initial_weights) + ['Final weight'] * len(final_weights)
        })
        ...
        ax = fig.gca()
        similarity_score = compute_ks_statistics(
            df, metric='Weight', min_n_sample=15)
        ...
        plt.savefig(os.path.join(
            boxplots_dir, f'{variable}.png'), dpi=300, transparent=False, bbox_inches='tight')
        plt.close()
    ...
    def pop_to_pop_weights_distribution(self, indices, variable='', voltage_scale=None):
        ...
        source_cell_types = [other_v1_utils.pop_name_to_cell_type(
            pop_name) for pop_name in source_pop_names]
        target_pop_names = other_v1_utils.pop_names(self.network)
        target_cell_types = [other_v1_utils.pop_name_to_cell_type(
            pop_name) for pop_name in target_pop_names]
        ...
        boxplots_dir = os.path.join(
            self.logdir, f'Boxplots/{variable}_distribution')
        os.makedirs(boxplots_dir, exist_ok=True)
        ...
        fig, axes = plt.subplots(
            num_rows, num_columns, figsize=(24, 5 * num_rows), sharey=True)
        ...
        for i, pre_name in enumerate(cell_type_order):
            ...
            similarity_score = compute_ks_statistics(
                subset_df, metric='Weight', min_n_sample=15)
            ...
            is_last_row = row_position == num_rows - \
                1  # or i >= num_pre_names - num_columns
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(
            boxplots_dir, f'{variable}.png'), dpi=300, transparent=False)
        plt.close()
        ...
        for layer in layer_order:
            if layer in layer_mapping and layer_mapping[layer]:
                ...
                fig, axes = plt.subplots(num_rows, num_cols, figsize=(
                    6*num_cols, 5*num_rows), squeeze=False)
                axes = axes.flatten()
                ...
                # Add super title
                fig.suptitle(
                    f'{descriptive_title} - {layer} Layer', fontsize=18, y=1.02)

                # Adjust layout
                plt.tight_layout()
                plt.savefig(os.path.join(
                    boxplots_dir, f'{variable}_{layer}.png'), dpi=300, transparent=False, bbox_inches='tight')
                plt.close()
    ...
    def pop_to_pop_weights_analysis(self, indices, variable='', voltage_scale=None):
        ...
        if not os.path.exists(os.path.join(self.logdir, 'Boxplots', variable, 'Initial_weight.png')):
            grouped_df = df.groupby(['Pre_names', 'Post_names'])[
                'Initial weight'].mean().reset_index()
            ...
            plt.title(f'Initial {descriptive_title}',
                      fontsize=title_fontsize, fontweight='bold')
            # Create a separate color bar axis closer to the heatmap
            cbar = plt.colorbar(
                heatmap.collections[0], ax=plt.gca(), pad=0.025, aspect=20)
        ...
        ### Final Weight ###
        grouped_df = df.groupby(['Pre_names', 'Post_names'])[
            'Final weight'].mean().reset_index()
        # Create a pivot table to reshape the data for the heatmap
        pivot_df = grouped_df.pivot(
            index='Pre_names', columns='Post_names', values='Final weight')
        # Plot heatmap
        ...
        plt.ylabel('Source Cell Type', fontsize=label_fontsize)
        plt.gca().set_aspect('equal')
        plt.title(f'Final {descriptive_title}',
                  fontsize=title_fontsize, fontweight='bold')
        # Plot color bar
        cbar = plt.colorbar(
            heatmap.collections[0], ax=plt.gca(), pad=0.025, aspect=20)
        ...
        ### Weight change ###
        grouped_df = df.groupby(['Pre_names', 'Post_names'])[
            'Weight Change'].mean().reset_index()
        # Create a pivot table to reshape the data for the heatmap
        try:
            pivot_df = grouped_df.pivot(
                index='Pre_names', columns='Post_names', values='Weight Change')
            # Plot heatmap
            plt.figure(figsize=(12, 6))
            heatmap = sns.heatmap(pivot_df, cmap='RdBu_r',
                                  annot=False, cbar=False, center=0)
            plt.xticks(rotation=90, fontsize=tick_fontsize)
            plt.yticks(fontsize=tick_fontsize)
            plt.xlabel('Target Cell Type', fontsize=label_fontsize)
            plt.ylabel('Source Cell Type', fontsize=label_fontsize)
            plt.gca().set_aspect('equal')
            plt.title(f'Changes in {descriptive_title}',
                      fontsize=title_fontsize, fontweight='bold')
            # Create a separate color bar axis closer to the heatmap
            cbar = plt.colorbar(
                heatmap.collections[0], ax=plt.gca(), pad=0.025, aspect=20)
    ...
    def fano_factor(self, spikes, t_start=0.7, t_end=2.5, n_samples=100, analyze_core_only=True):
        ...
        if analyze_core_only:
            ...
            if spikes.shape[3] != n_neurons and spikes.shape[3] == len(core_mask):
                ...
                raise ValueError(
                    f"Spikes tensor shape mismatch: expected {n_neurons} neurons, but got {spikes.shape[3]} neurons in the spikes tensor.")
    ...
    def fanos_figure(self, spikes, n_samples=100, spont_fano_duration=300, evoked_fano_duration=300, analyze_core_only=True, data_dir='Synchronization_data'):
        ...
        evoked_exp_data_path = os.path.join(
            data_dir, 'Fano_factor_v1', f'v1_fano_running_{evoked_fano_duration}ms_evoked.npy')
        evoked_exp_fanos = np.load(evoked_exp_data_path, allow_pickle=True)
        spont_exp_data_path = os.path.join(
            data_dir, 'Fano_factor_v1', f'v1_fano_running_{spont_fano_duration}ms_spont.npy')
        ...
        evoked_exp_fanos_mean = np.nanmean(evoked_exp_fanos, axis=0)[
            :len(evoked_bin_sizes)]
        evoked_exp_fanos_std = np.nanstd(evoked_exp_fanos, axis=0)[
            :len(evoked_bin_sizes)]
        evoked_exp_fanos_sem = evoked_exp_fanos_std / \
            np.sqrt(evoked_exp_fanos.shape[0])
        spont_exp_fanos_mean = np.nanmean(spont_exp_fanos, axis=0)[
            :len(spont_bin_sizes)]
        spont_exp_fanos_std = np.nanstd(spont_exp_fanos, axis=0)[
            :len(spont_bin_sizes)]
        spont_exp_fanos_sem = spont_exp_fanos_std / \
            np.sqrt(spont_exp_fanos.shape[0])
        ...
        # Increased from (12, 5) for better visibility
        ...
        ax1.axhline(y=1.0, color=poisson_color, linestyle='--',
                    alpha=0.8, linewidth=2.0, zorder=2)
        ax1.text(0.75*evoked_bin_sizes[-1], 1.05, 'Poisson',
                 color=poisson_color, fontsize=TEXT_SIZE)

        # Configure subplot
        ax1.set_xscale("log")
        ax1.set_xlabel('Bin Size (s)', fontsize=AXIS_LABEL_SIZE,
                       fontweight='bold')
        ax1.set_ylabel('Fano Factor', fontsize=AXIS_LABEL_SIZE,
                       fontweight='bold')
        ...
        upper_y_limit = max(
            [2, max(np.max(evoked_fanos_mean), np.max(evoked_exp_fanos_mean)) * 1.2])
        ...
        # Add horizontal line at Fano=1 for Poisson reference
        ax2.axhline(y=1.0, color=poisson_color, linestyle='--',
                    alpha=0.8, linewidth=2.0, zorder=2)
        ax2.text(0.75*spont_bin_sizes[-1], 1.05, 'Poisson',
                 color=poisson_color, fontsize=TEXT_SIZE)

        # Configure subplot
        ax2.set_xscale("log")
        ax2.set_xlabel('Bin Size (s)', fontsize=AXIS_LABEL_SIZE,
                       fontweight='bold')
    ...
    def plot_population_firing_rates_vs_tuning_angle(self, spikes, DG_angles, analyze_core_only=True):
        ...
        if analyze_core_only:
            core_mask = other_v1_utils.isolate_core_neurons(
                self.network, radius=self.flags.plot_core_radius, data_dir=self.flags.data_dir)
            if spikes.shape[3] != np.sum(core_mask) and spikes.shape[3] == len(core_mask):
                spikes = spikes[:, :, :, core_mask]

            tuning_angles = tuning_angles[core_mask]

        spikes = np.sum(spikes, axis=0).astype(
            np.float32)/self.flags.n_trials_per_angle
        seq_len = spikes.shape[1]

        for angle_id, angle in enumerate(DG_angles):
            firingRates = calculate_Firing_Rate(
                spikes[angle_id, :, :], stimulus_init=self.pre_delay, stimulus_end=seq_len-self.post_delay, temporal_axis=0)
            x = tuning_angles
            y = firingRates
            ...
            plt.savefig(os.path.join(
                spikes_dir, f'v1_spikes_angle_{angle}.png'))
            plt.close()
    ...
    def osi_dsi_analysis(self, v1_spikes, DG_angles):
        ...
        psd_analyzer(v1_spikes, t_spont_start=0, t_spont_end=self.pre_delay,
                     t_evoked_start=self.pre_delay+0, t_evoked_end=stimulus_duration-self.post_delay)
        ...
        t_fano0 = time()
        self.fanos_figure(v1_spikes, n_samples=1000, spont_fano_duration=300,
                          evoked_fano_duration=300, analyze_core_only=True)
        self.fanos_figure(v1_spikes, n_samples=1000, spont_fano_duration=1800,
                          evoked_fano_duration=1800, analyze_core_only=True)
        print('Fanos figure saved in ', time() - t_fano0, ' seconds!\n')
        # # Plot the tuning angle analysis
        self.plot_population_firing_rates_vs_tuning_angle(
            v1_spikes, DG_angles, analyze_core_only=True)
        ...
        metrics_analysis = ModelMetricsAnalysis(v1_spikes, DG_angles, self.network, data_dir=self.flags.data_dir,
                                                drifting_gratings_init=self.pre_delay, drifting_gratings_end=self.pre_delay +
        ...
        metrics_analysis(metrics=[
                         "Evoked rate (Hz)"], directory=fr_boxplots_dir, filename=f'Epoch_{self.current_epoch}')
        # Spontaneous rates figure
        metrics_analysis(metrics=['Spontaneous rate (Hz)'],
                         directory=spontaneous_boxplots_dir, filename=f'Epoch_{self.current_epoch}')
...
class Callbacks:
    def __init__(self, network, lgn_input, bkg_input, model, optimizer, flags, logdir, strategy,
        ...
        if checkpoint is None:
            ...
            data_loaded, _ = tf_utils.load_training_state(
                self.logdir, restore_from=flags.restore_from
            )
            if data_loaded is not None:
                ...
                self.checkpoint_epochs = len(
                    data_loaded['epoch_metric_values']['train_loss'])
            else:
    ...
    def on_train_end(self, metric_values, normalizers=None):
        ...
        print("\n ---------- Training ended at ",
              dt.datetime.now().strftime('%d-%m-%Y %H:%M'), ' ----------\n')
        print(
            f"Total time spent: {self.train_end_time - self.train_start_time:.2f} seconds")
        print(
            f"Average step time: {np.mean(self.step_running_time):.2f} seconds\n")
        ...
        print(
            f"|{'-' * (max_key_length + 2)}|{'-' * (max_key_length + 2)}|{'-' * (max_key_length + 2)}|")
        ...
        sem_step_time = np.std(self.step_running_time) / \
            np.sqrt(len(self.step_running_time))
        ...
        effective_seq_len = training_utils.infer_effective_sequence_length(self.flags)
        stats_file = tf_utils.append_performance_statistics(
            logdir=self.logdir,
            n_neurons=self.n_neurons,
            n_edges=self.n_edges,
    ...
    def on_step_end(self, train_values, y, verbose=True):
        ...
        if verbose:
            ...
            tf.print(print_str)
            print(
                f'    Step running time: {time() - self.step_init_time:.2f}s')
            for gpu_id in range(len(self.strategy.extended.worker_devices)):
                printgpu(gpu_id=gpu_id)
    ...
    def plot_raster(self, x, v1_spikes, y, stimulus_type='evoked'):
        seq_len = v1_spikes.shape[1]
        ...
        graph = InputActivityFigure(
            ...
            self.flags.data_dir,
            images_dir,
            filename=f'Epoch_{self.epoch}_{stimulus_type}',
            frequency=self.flags.temporal_f,
            stimuli_init_time=self.pre_delay,
            stimuli_end_time=seq_len-self.post_delay,
            reverse=False,
    ...
    def plot_lgn_activity(self, x, x_spont):
        ...
        os.makedirs(os.path.join(
            self.logdir, 'Populations activity'), exist_ok=True)
        plt.savefig(os.path.join(self.logdir, 'Populations activity',
                    f'LGN_population_activity_epoch_{self.epoch}.png'))
        plt.close()
    ...
    def plot_mean_firing_rate_boxplot(self, v1_spikes, y):
        ...
        DG_angles = y
        boxplots_dir = os.path.join(
            self.logdir, f'Boxplots/{self.neuropixels_feature}')
        os.makedirs(boxplots_dir, exist_ok=True)
        if self.neuropixels_feature == "Evoked rate (Hz)":
            ...
            metrics_analysis = ModelMetricsAnalysis(v1_spikes, DG_angles, self.network, data_dir=self.flags.data_dir,
                                                    ...
                                                    neuropixels_df=self.flags.neuropixels_df)
        # Figure for Average firing rate boxplots
        metrics_analysis(metrics=[self.neuropixels_feature],
