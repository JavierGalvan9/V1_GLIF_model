...
def main(_):
    ...
    # if socket.gethostname().count('nvcluster') > 0 or socket.gethostname().count('pCluster') > 0 or socket.gethostname().count('n') > 0:
    ...
    if task_name == 'drifting_gratings':
        with open(os.path.join(flags.data_dir, 'np_gratings_firing_rates.pkl'), 'rb') as f:
            firing_rates = pkl.load(f)
        # this firing rates is a dictionary of dictionaries.
        # I can ignore keys.
        # I want to know the number of neurons in each cell type at this point.
        # OK. maybe it's hard without actual network, so I'll just postpone it.
    else:
    ...
    if task_name == '10class':
        n_output = 10
    elif task_name == 'drifting_gratings':
        n_output = 2
    else:
    ...
    with strategy.scope():
        ...
        if task_name == 'drifting_gratings':
            # postponed process. Let's make the target firing rates here.
            # firing_rates are already defined.
            
            # detect the number of neurons in each cell type
...
if __name__ == '__main__':
    ...
    # absl.app.flags.DEFINE_string('restore_from', '../results/multi_training/b_xdk7/results/ckpt-8', '')
    # absl.app.flags.DEFINE_string('restore_from', '../results/multi_training/b_hl3u/results/ckpt-5', '')
    # absl.app.flags.DEFINE_string('restore_from', '../results/multi_training/b_hcwx/results/ckpt-15', '')
    # absl.app.flags.DEFINE_string('restore_from', '../results/multi_training/b_wmi0/results/ckpt-25', '')
    # absl.app.flags.DEFINE_string('restore_from', '../results/multi_training/b_x9qt/results/ckpt-50', '')
    ...
    # absl.app.flags.DEFINE_integer('n_epochs', 16, '')
    absl.app.flags.DEFINE_integer('n_epochs', 51, '')
    absl.app.flags.DEFINE_integer('batch_size', 1, '')
    # absl.app.flags.DEFINE_integer('neurons', 230924, '')
    absl.app.flags.DEFINE_integer('neurons', 66634, '')
    # absl.app.flags.DEFINE_integer('neurons', 10000, '')
    # absl.app.flags.DEFINE_integer('neurons', 2000, '')
    absl.app.flags.DEFINE_integer('seq_len', 600, '')
    # absl.app.flags.DEFINE_integer('seq_len', 100, '')
    ...
    absl.app.flags.DEFINE_integer('steps_per_epoch', 100, '')# EA and garret dose not need this many but pure classification needs 781 = int(50000/64)
