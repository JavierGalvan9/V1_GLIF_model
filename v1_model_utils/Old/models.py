...
class V1Column(tf.keras.layers.Layer):
    def __init__(
        ...
        path='GLIF_network/network/tau_basis.npy'
        ...
        self.input_tau_syn_weights = input_population["tau_syn_weights_array"]
        ...
        # )
        print('bkg_weights_shape', bkg_weights.shape)
        print(bkg_weights[0, 0])
        print(bkg_weights[0, 1])

        # divide every bkg_weights axis 1 element by voltage_scale[self._node_type_ids]
        bkg_weights = bkg_weights /voltage_scale[self._node_type_ids][:, np.newaxis]
...
if __name__ == "__main__":
    ...
    input_population, network, bkg, bkg_weights = load_sparse.cached_load_v1(
        n_input,
        n_neurons,
        True,
        "GLIF_network",
