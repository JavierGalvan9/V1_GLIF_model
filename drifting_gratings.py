...
import toolkit
from plotting_utils import (InputActivityFigure, LaminarPlot, LGN_sample_plot,
                            PopulationActivity, RasterPlot)
...
def main(_):
    ...
    path = os.path.join(flags.save_dir, f'orien_{str(flags.gratings_orientation)}_freq_{str(flags.gratings_frequency)}_reverse_{str(flags.reverse)}_rec_{flags.neurons}')
    ...
    input_population, network, bkg, bkg_weights = load_fn(
        flags.n_input, flags.neurons, flags.core_only, flags.data_dir, flags.seed, flags.connected_selection, flags.n_output,
    ...
    full_selection_data_path = os.path.join(path, 'Network_simulation_results', 'Input_current', 'e23_classification_base_keller.lzma')
    if flags.output_currents and os.path.exists(full_selection_data_path):
        model = models_output_currents.create_model(
            network, input_population, bkg_weights, full_selection_data_path, seq_len=flags.seq_len, n_input=flags.n_input,
            n_output=flags.n_output, cue_duration=flags.recall_duration, dtype=dtype,
            ...
            use_state_input=True, return_state=True, hard_reset=flags.hard_reset)
        ...
        model = models.create_model(
            network, input_population, bkg_weights, seq_len=flags.seq_len, n_input=flags.n_input,
            n_output=flags.n_output, cue_duration=flags.recall_duration, dtype=dtype,
            input_weight_scale=input_weight_scale, dampening_factor=flags.dampening_factor,
            gauss_std=flags.gauss_std, lr_scale=flags.lr_scale, train_recurrent=flags.train_recurrent,
            neuron_output=flags.neuron_output, recurrent_dampening_factor=flags.recurrent_dampening_factor,
    ...
    time_per_sim = 0
    time_per_save = 0
    keys = ['z', 'v', 'input_current', 'recurrent_current', 'bottom_up_current', 'z_lgn']
    ...
    SimulationDataHDF5 = other_billeh_utils.SaveSimDataHDF5(flags, keys, data_path, network,
                                                            save_core_only=True, dtype=save_dtype)
    ...
    for trial in range(0, flags.n_simulations):
        ...
        inputs = (np.random.uniform(size=inputs.shape,
                                    low=0., high=1.) < firing_rates * .001).astype(np.uint8)
        t_init_sim = time()
        ...
        if flags.output_currents:
            ...
            simulation_data = {'z':z, 'v':v, 'input_current':input_current, 
    ...
    graph = InputActivityFigure(network, flags.data_dir, image_path, filename=raster_filename, 
                                frequency=flags.gratings_frequency,  stimuli_init_time=500, stimuli_end_time=1500,
                                reverse=flags.reverse, plot_core_only=True)
    graph(inputs, z)
    # Plot LGN units firing rates
    LGN_units = LGN_sample_plot(firing_rates, inputs, stimuli_init_time=500,
                                stimuli_end_time=1500, images_dir=image_path, n_samples=2)
    LGN_units()
    # Plot the mean firing rate of the population of neurons
    Population_activity = PopulationActivity(n_neurons=flags.neurons, network=network, 
                                              image_path=image_path, data_dir=flags.data_dir)
...
if __name__ == '__main__':
    ...
    absl.app.flags.DEFINE_integer('neurons', 5000, '')  # 230,924 to take all neurons, 51,978 to take only core neurons
    ...
    absl.app.flags.DEFINE_boolean('output_currents', True, '')
