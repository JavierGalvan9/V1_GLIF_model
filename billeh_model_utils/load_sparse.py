...
def load_network(path='GLIF_network/network/network_dat.pkl',
...
def load_input(path='GLIF_network/network/input_dat.pkl',
               start=0,
               duration=3000,
               dt=1,
    ...
    for input_population in d:
        ...
        for idx, edge in enumerate(input_population):  # input_population[1]:
            # Identify the which of the 10 types of inputs we have
            r = edge['params']['receptor_type'] - 1
            # r takes values within 0 - 9
            ...
            post_indices.extend(10 * target_tf_id + r)
        ...
        input_populations.append(dict(n_inputs=n_neurons, indices=indices.astype(
            np.int64), weights=weights, delays=delays))
...
def load_billeh(n_input, n_neurons, core_only, data_dir, seed=3000, connected_selection=False, n_output=2,
    ...
    inputs = load_input(
        start=1000, duration=1000, dt=1, path=os.path.join(data_dir, 'network/input_dat.pkl'),
    ...
    bkg = inputs[1]
    bkg_weights = np.zeros((network['n_nodes'] * 10,), np.float32)
