...
def plot_network_edges_impact(df, output_dir):
    """
    Create figures showing the impact of network edges (n_edges) on performance metrics.
    ...
    # Create a figure showing the relationship between n_edges and step time
    ...
    if not train_df.empty:
        ...
        # Add regression line
        x = train_df['n_edges']
        y = train_df['step_time']
        z = np.polyfit(x, y, 1)
        p = np.poly1d(z)
        plt.plot(sorted(x), p(sorted(x)), "r--", alpha=0.8, 
                 label=f"Linear fit (y = {z[0]:.2e}x + {z[1]:.2f})")
        
    plt.title('A) Network Edges vs Step Time (Training Mode)', fontweight='normal', loc='left')
    ...
    plt.title('B) Network Edges vs Step Time (Testing Mode)', fontweight='normal', loc='left')
    plt.xlabel('Number of Network Edges')
    plt.ylabel('Step Time (s)')
    plt.grid(alpha=0.3, linestyle='--')
    plt.legend(title='Batch Size')
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, 'network_edges_step_time.png'), dpi=300)
    plt.close()
    
    # Create a figure showing the relationship between n_edges and memory usage
    plt.figure(figsize=(10, 7))
    ...
    plt.title('A) Network Edges vs Memory Usage (Training Mode)', fontweight='normal', loc='left')
    plt.xlabel('Number of Network Edges')
    ...
    plt.title('B) Network Edges vs Memory Usage (Testing Mode)', fontweight='normal', loc='left')
    plt.xlabel('Number of Network Edges')
    plt.ylabel('GPU Memory (GB)')
    plt.grid(alpha=0.3, linestyle='--')
    plt.legend(title='Batch Size')
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, 'network_edges_memory.png'), dpi=300)
    plt.close()
    
    # Create a heatmap showing relationship between n_neurons, n_edges, and step time
    plt.figure(figsize=(12, 5))
    
    # Logarithmic values for better visualization
    for mode_df, mode_name, subplot_pos in [(train_df, 'Training', 121), (test_df, 'Testing', 122)]:
        if not mode_df.empty:
            plt.subplot(subplot_pos)
            
            # Calculate edge density (edges per neuron)
            mode_df['edge_density'] = mode_df['n_edges'] / mode_df['neurons']
            
            # Create pivot table for neurons vs edges/neuron heatmap
            ...
            sns.heatmap(
                ...
                fmt=".2f",
                cmap="YlGnBu",
                cbar_kws={'label': 'Step Time (s)'}
            )
            
            plt.title(f'Neuron Count vs Batch Size ({mode_name} Mode)', fontweight='normal')
            plt.ylabel('Number of Neurons')
            plt.xlabel('Batch Size')
    
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, 'neuron_vs_batch_heatmap.png'), dpi=300)
    plt.close()
    
    # Create a figure showing the relationship between n_edges and n_neurons with step time
    plt.figure(figsize=(10, 8))
    
    # Plot for both modes if available
    for i, (mode_df, mode_name) in enumerate([(train_df, 'Training'), (test_df, 'Testing')]):
        if not mode_df.empty:
            plt.subplot(2, 1, i+1)
            
            # Create scatter plot with size representing step time
            scatter = plt.scatter(
                mode_df['neurons'],
                mode_df['n_edges'],
                c=mode_df['step_time'],
                s=mode_df['step_time'] * 20,  # Size proportional to step time
                alpha=0.7,
                cmap='viridis'
            )
            ...
            cbar.set_label('Step Time (s)')
            
            # Add text annotations for selected points
            for idx, row in mode_df.iloc[::max(1, len(mode_df)//10)].iterrows():  # Annotate every ~10th point
                plt.annotate(
                    f"b={row['batch_size']}, s={row['seq_len']}",
                    (row['neurons'], row['n_edges']),
                    xytext=(5, 5),
                    textcoords='offset points',
                    fontsize=7,
                    alpha=0.8
            ...
            plt.title(f'{mode_name} Mode: Neurons vs Edges (color = step time)', fontweight='normal', loc='left')
            plt.xlabel('Number of Neurons')
            plt.ylabel('Number of Network Edges')
            plt.grid(alpha=0.3, linestyle='--')
            plt.xscale('log')
            plt.yscale('log')
    
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, 'neurons_vs_edges.png'), dpi=300)
    plt.close()
...
def create_normalized_step_time_comparison(train_df, test_df, output_dir):
    ...
    # Filter for sequence length 4000 in training data
    train_seq4000 = train_df[train_df['seq_len'] == 4000] if 'seq_len' in train_df.columns else pd.DataFrame()
    ...
    if not train_seq4000.empty:
        train_seq4000['normalized_step_time'] = train_seq4000['step_time'] / train_seq4000['batch_size']
    
    if not test_data.empty:
        test_data['normalized_step_time'] = test_data['step_time'] / test_data['batch_size']
    
    # Create figure for comparison
    if not train_seq4000.empty and not test_data.empty:
        ...
        common_neurons = sorted(set(train_seq4000['neurons'].unique()) & set(test_data['neurons'].unique()))
        ...
        for neuron in common_neurons:
            ...
            train_neurons = train_seq4000[train_seq4000['neurons'] == neuron]
        ...
        plt.bar(x - width/2, train_norm_times, width, label='Training (seq_len=4000)', color='royalblue')
        ...
        plt.plot(common_neurons, train_norm_times, 'o-', label='Training (seq_len=4000)', color='royalblue')
...
def create_train_test_comparison(train_df, test_df, output_dir):
    ...
    # =========================================================================
    # FIGURE 5: Firing Rate Comparison (if available)
    # =========================================================================
    if 'mean_rate' in train_df.columns and 'mean_rate' in test_df.columns:
        plt.figure(figsize=(12, 6))
        
        # Calculate average firing rates for each neuron count
        train_rates = []
        test_rates = []
        train_rates_err = []
        test_rates_err = []
        ...
        for neuron in common_neurons:
            ...
            # Calculate mean and standard error
            train_rates.append(train_subset['mean_rate'].mean())
            test_rates.append(test_subset['mean_rate'].mean())
            train_rates_err.append(train_subset['mean_rate'].sem() if 'sem_rate' in train_subset.columns else 0)
            test_rates_err.append(test_subset['mean_rate'].sem() if 'sem_rate' in test_subset.columns else 0)
        
        # Create bar chart
        plt.bar(x - width/2, train_rates, width, yerr=train_rates_err,
               label='Training', color='royalblue', capsize=5)
        plt.bar(x + width/2, test_rates, width, yerr=test_rates_err,
               label='Testing', color='darkorange', capsize=5)
        
        # Add difference annotations
        for i, (train, test) in enumerate(zip(train_rates, test_rates)):
            diff_pct = abs(train - test) / max(train, test) * 100
            plt.text(x[i], max(train, test) * 1.05, 
                    f"{diff_pct:.1f}% diff", 
                    ha='center', fontsize=9)
        ...
        plt.title('Firing Rate Comparison: Training vs Testing', fontweight='normal')
        plt.xticks(x, [f"{n:,}" for n in common_neurons], rotation=45)
        plt.grid(alpha=0.3, linestyle='--', axis='y')
        plt.legend()
        
        plt.tight_layout()
        plt.savefig(os.path.join(output_dir, 'firing_rate_comparison.png'), dpi=300)
        plt.close()
...
def main():
    ...
    parser.add_argument("--csv", default="/home/jgalvan/Desktop/Neurocoding/V1_GLIF_model/Simulation_results/performance_statistics.csv", 
                       help="Path to CSV file containing training statistics")
    parser.add_argument("--output-dir", default="performance_figures", help="Directory to save generated figures")
    ...
    if 'mode' in df.columns:
        ...
        if not train_df.empty:
            ...
            # Plot network edges impact for training
            plot_network_edges_impact(train_df, os.path.join(base_output_dir, "network_edges_train"))
        ...
        # Plot network edges impact
        plot_network_edges_impact(df, os.path.join(base_output_dir, "network_edges"))
            
    print("\nPerformance analysis complete!")
    print(f"All figures have been saved to {base_output_dir}/")
    print("Summary of changes:")
    print("- Added support for train/test mode separation")
    print("- Created simplified visualizations for test data")
    print("- Generated train vs test comparison with normalized step time")
    print("- Added visualizations for network edges (n_edges) and firing rates (mean_rate)")
